...
export default function AlertList({alerts}:{alerts:{id:string,severity:string,message:string}[]}){
  return (
    <div className="space-y-2">
      ...
      {alerts.map(a=> (
        <div key={a.id} className={`rounded-2xl border p-4 ${a.severity==='critical' ? 'border-[rgba(255,77,141,0.24)] bg-[rgba(255,77,141,0.12)]' : 'border-white/8 bg-white/5'}`}>
          <div className="flex items-center justify-between gap-3">
            <div className="text-xs uppercase tracking-[0.24em] text-slate-400">{a.severity.toUpperCase()}</div>
            <div className={`h-2 w-2 rounded-full ${a.severity==='critical' ? 'bg-[var(--color-neon)]' : 'bg-[var(--color-apex)]'}`} />
          </div>
